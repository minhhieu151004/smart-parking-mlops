...
def model_fn(model_dir):
    """
    SageMaker gọi 1 lần khi load model.
    Phải return đúng duy nhất object model.
    """
...
def input_fn(request_body, content_type):
    ...
    """
    print(f"[input_fn] Content type = {content_type}")
...
def _preprocess(df):
    """
    Resample + scale + lấy sequence đầu vào cho LSTM
    """
...
def predict_fn(df, model):
    ...
    X, last_timestamp = _preprocess(df)
    ...
    print("[predict_fn] Result:", response)
    return response
