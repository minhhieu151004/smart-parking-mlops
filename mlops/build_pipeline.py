...
sklearn_processor_consolidate = SKLearnProcessor(
    framework_version="1.2-1", role=role, instance_type=processing_instance_type,
    instance_count=1, base_job_name="consolidate-data-job", sagemaker_session=sagemaker_session,
)
step_consolidate_data = ProcessingStep(
    ...
    inputs=[ProcessingInput(source=baseline_data_uri, destination="/opt/ml/processing/input_baseline")],
    outputs=[ProcessingOutput(output_name="new_baseline_data", source="/opt/ml/processing/output", destination=consolidated_output_s3_uri)],
...
tf_estimator = TensorFlow(
    ...
    hyperparameters={"epochs": training_epochs, "learning-rate": 0.001, "model-version": model_version},
...
step_train_model = TrainingStep(
    ...
    inputs={"training": TrainingInput(s3_data=step_consolidate_data.properties.ProcessingOutputConfig.Outputs["new_baseline_data"].S3Output.S3Uri, distribution="FullyReplicated", content_type="text/csv", s3_data_type="S3Prefix")}
)
...
model = TensorFlowModel(
    ...
    entry_point="inference.py", # Tên file
    source_dir=CODE_DIR         # Đường dẫn tuyệt đối tới thư mục mlops/code/
)

model_metrics = ModelMetrics(
    model_statistics=MetricsSource(
        s3_uri=Join(on='/', values=[model_output_s3_uri, step_train_model.properties.TrainingJobName, "output/data", "metrics.json"]),
...
step_conditional_retrain_flow = ConditionStep(name="DriftCheckCondition", conditions=[condition_drift_detected], if_steps=[step_consolidate_data, step_train_model, step_register_model], else_steps=[])

pipeline = Pipeline(
    name=pipeline_name,
    parameters=[baseline_data_uri, daily_data_bucket, actual_data_prefix, prediction_data_prefix, model_mae_drift_threshold, p_value_drift_threshold, training_epochs, processing_instance_type, training_instance_type],
